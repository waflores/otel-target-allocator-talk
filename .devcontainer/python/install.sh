    ...
    case ${ADJUSTED_ID} in
        debian)
            if ! dpkg -s "$@" > /dev/null 2>&1; then
                pkg_mgr_update
                ${INSTALL_CMD} "$@"
                # XXX We need to bypass ssl GIT_SSL_NO_VERIFY=true
                updaterc "export GIT_SSL_NO_VERIFY=true"
                updaterc "export SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt"
                updaterc "export CURL_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt"
                updaterc "export NIX_SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt"
                updaterc "export SSL_CERT=/etc/ssl/certs/ca-certificates.crt"
