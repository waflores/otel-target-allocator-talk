...
# XXX We have to bypass ssl
export NODE_EXTRA_CA_CERTS=/etc/ssl/certs/ca-certificates.crt
export GIT_SSL_NO_VERIFY=true
export SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt
export SSL_CERT=/etc/ssl/certs/ca-certificates.crt
...
if [ "${DOCKER_DASH_COMPOSE_VERSION}" != "none" ]; then
    ...
    if [ "${DOCKER_DASH_COMPOSE_VERSION}" = "v1" ]; then
        ...
        curl --insecure -fsSL "https://github.com/docker/compose/releases/download/v${compose_version}/docker-compose-linux-${target_compose_arch}" -o ${docker_compose_path} || {
        ...
        # Download the SHA256 checksum
        DOCKER_COMPOSE_SHA256="$(curl --insecure -sSL "https://github.com/docker/compose/releases/download/v${compose_version}/docker-compose-linux-${target_compose_arch}.sha256" | awk '{print $1}')"
        echo "${DOCKER_COMPOSE_SHA256}  ${docker_compose_path}" > docker-compose.sha256sum
        sha256sum -c docker-compose.sha256sum --ignore-missing
...
if [ "${INSTALL_DOCKER_COMPOSE_SWITCH}" = "true" ] && ! type compose-switch > /dev/null 2>&1; then
    if type docker-compose > /dev/null 2>&1; then
        ...
        curl --insecure -fsSL "https://github.com/docker/compose-switch/releases/download/v${compose_switch_version}/docker-compose-linux-${architecture}" -o /usr/local/bin/compose-switch || fallback_compose-switch "$compose_switch_url"
...
fallback_buildx() {
    ...
    wget --no-check-certificate https://github.com/docker/buildx/releases/download/v${buildx_version}/${buildx_file_name}
}
...
if [ "${INSTALL_DOCKER_BUILDX}" = "true" ]; then
    ...
    cd /tmp
    wget --no-check-certificate https://github.com/docker/buildx/releases/download/v${buildx_version}/${buildx_file_name} || fallback_buildx "$docker_buildx_url"
